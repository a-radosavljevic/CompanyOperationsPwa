...
const Upload = () => {
  ...
  const handleDataChange = (e) =>
    (documentObj.current[e.target.name] = e.target.value);
  ...
  const submitDocument = async () => {
    ...
    formData.append(
      "content",
      new Blob([documentObj.current.content]),
      "filename.bin"
    );
    ...
    <>
      <MainContainer className="slide-up-anim">
        ...
        {(takePhotoVisible || uploadVisible) && (
          <>
            <button className="btn btn-light" onClick={cancelAdding}>
              <FontAwesomeIcon icon={solid("chevron-left")} /> Otkaži dodavanje
              dokumenta
            </button>
            <UploadRow>
              <UploadDiv>
                {takePhotoVisible && (
                  <Camera handleTakingPhoto={handleUpload} errors={errors} />
                )}
                {uploadVisible && (
                  <FileSubmit handleUpload={handleUpload} errors={errors} />
                )}
              </UploadDiv>
              <UploadDiv>
                <DocumentData
            ...
            <div className="text-right slide-up-anim">
              <button onClick={submitDocument} className="btn btn-primary block-btn">
                Sačuvaj
              </button>
            </div>
          </>
        ...
        {!takePhotoVisible && !uploadVisible && (
          <UploadContainer
            chooseTakePhoto={() => setTakingPhotoVisible(true)}
            chooseUpload={() => setUploadVisible(true)}
          ></UploadContainer>
        )}
      </MainContainer>
    </>
