...
const AddDocumentsInWorkflow = () => {
  ...
  const fetchTaskDocuments = async () => {
    let response = await http.get(
      `workflow/get-documents?workflowId=${taskId}`
    );
    if (response.status === 200 || response.status === 204) {
      if (response.data) setDocumentIds(response.data);
    } else {
      ErrorMessage("Došlo je do neočekivane greške", "", () =>
        window.location.reload()
      );
  ...
  const finishAdding = async () => {
    ...
    let response = await http.post(
      "workflow/add-documents",
      submitObject.current
    );
    if (response.status === 200) {
      SuccessMessage(
        "Dokumenti su uspešno dodati u radni zadatak",
        "",
        () => (window.location.href = `/workflow-task?id=${taskId}`)
      );
    } else {
      ErrorMessage("Došlo je do neočekivane greške", "", () =>
        window.location.reload()
  ...
  return (
    <>
      ...
      <div className="text-right">
        <MainContainer className="slide-up-anim">
          <button className="btn btn-primary block-btn" onClick={finishAdding}>
            Završi dodavanje
          </button>
        </MainContainer>
      </div>
