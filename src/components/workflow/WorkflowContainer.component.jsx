...
const WorkflowContainer = ({
  ...
  const columns = [
    ...
    },
    {
      accessor: "status",
      Cell: ({ row }) => {
        return workflowStatusList.find((x) => x.value === row.original.status)
          .label;
      },
    },
    {
        ...
        return (
          <>
            <Link
              ...
              to={`/workflow-task?id=${row.original.id}`}
            >
              <FontAwesomeIcon icon={solid("check")} />
            </Link>
            <a
              onClick={() => rejectWorkflow(row.original.id)}
    ...
    <>
      <MainContainer className="slide-up-anim">
        ...
        <div className="flex-btn-div-2">
          ...
          <button className="btn btn-primary" onClick={searchMyWorkflows}>
            Pretraga
          </button>
        </div>
      </MainContainer>
      ...
      <Modal
        show={showStartWorkflow}
        title={"Novi radni zadatak"}
        handleClose={() => setShowStartWorkflow(false)}
      >
